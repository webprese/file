<?php
require_once("flowline-customer-reviews-additional.php");
class WPCustomerReviewsAdmin
{
	...
	function add_meta_box() {
		...
		$meta_box = array(
			'id' => 'wpcr-meta-box',
			'title' => '<img src="'.$this->parentClass->getpluginurl().'star.png" />&nbsp;Flowline Customer Reviews',
			'page' => 'page',
			'context' => 'normal',
			'priority' => 'high',
			'fields' => array(
				array(
					'name' => '<span style="font-weight:bold;">Enable Flowline Customer Reviews</span> for this page',
	...
	function enqueue_admin_stuff() {
		...
		if (isset($this->p->page) && ( $this->p->page == 'wpcr_view_reviews' || $this->p->page == 'wpcr_options' ) ) {
			...
			wp_register_script('flowline-customer-reviews-admin',$pluginurl.'flowline-customer-reviews-admin.js',array(),$this->plugin_version);
			wp_enqueue_script('flowline-customer-reviews-admin');
			wp_register_style('flowline-customer-reviews',$pluginurl.'flowline-customer-reviews.css',array(),$this->plugin_version);        
			wp_enqueue_style('flowline-customer-reviews');
			wp_register_style('flowline-customer-reviews-admin',$pluginurl.'flowline-customer-reviews-admin.css',array(),$this->plugin_version);        
			wp_enqueue_style('flowline-customer-reviews-admin');
		}
	}
    ...
    function notify_activate($act_flag) {
        ...
        $host = "www.flowlinedesigns.com";
        $port = 80;
		$wpurl = get_bloginfo('wpurl');
        
        $http_request  = "POST /plugin-activation/activate.php HTTP/1.0\r\n";
        $http_request .= "Host: www.flowlinedesigns.com\r\n";
	...
	function update_options() {
        ...
        {	
			...
			$updated_options['facebook_apisecret'] = $this->p->facebook_apisecret;
			$updated_options['facebook_apiappid'] = $this->p->facebook_apiappid;
			$updated_options['facebook_apisessionkey'] = $this->p->facebook_apisessionkey;
			$updated_options['facebook_apiuserid'] = $this->p->facebook_apiuserid;
			$updated_options['facebook_apipageid'] = $this->p->facebook_apipageid;
			$updated_options['facebook_publishtoprofile'] = intval($this->p->facebook_publishtoprofile);
			$updated_options['facebook_publishtopage'] = intval($this->p->facebook_publishtopage);
			$updated_options['twitter_ckey'] = $this->p->twitter_ckey;
			$updated_options['twitter_csecret'] = $this->p->twitter_csecret;
			$updated_options['autotweet_name'] = $this->p->autotweet_name;
			$updated_options['autotweet_token'] = $this->p->autotweet_token;
			$updated_options['autotweet_secret'] = $this->p->autotweet_secret;
			$updated_options['autotweet_flag'] = $this->p->autotweet_flag;
			if(!$this->p->show_date)
				$updated_options['show_date'] = 0;
			else
				$updated_options['show_date'] = $this->p->show_date;
			if(!$this->p->show_title)
				$updated_options['show_title'] = 0;
			else
				$updated_options['show_title'] = $this->p->show_title;
			if(!$this->p->custom_citystatecountry)
				$updated_options['custom_citystatecountry'] = 0;
			else
				$updated_options['custom_citystatecountry'] = $this->p->custom_citystatecountry;
			if(!$this->p->custom_anonymous)
				$updated_options['custom_anonymous'] = 0;
			else
				$updated_options['custom_anonymous'] = $this->p->custom_anonymous;
	...
	function show_options() {
		...
		$facebook_publishtoprofile="";
		$facebook_publishtopage="";

		if( $this->options['facebook_publishtoprofile']==1 ) 
			$facebook_publishtoprofile="checked";
		if( $this->options['facebook_publishtopage']==1 ) 
			$facebook_publishtopage="checked";

		ob_start();
		wpcr_publish_extended_callback();
		$wpcr_facebookconnect = ob_get_contents();
		ob_end_clean();
		
		ob_start();
		wpcr_twitter_extended_callback();
		$wpcr_twitterconnect = ob_get_contents();
		ob_end_clean();
		/*
		ob_start();
		wpcr_linkedin_extended_callback();
		$wpcr_linkedinconnect = ob_get_contents();
		ob_end_clean();
		*/
		$crf_showdate="";
		$crf_showtitle="";
		if($this->options['show_date'])
	        $crf_showdate="checked";
		if($this->options['show_title'])
	        $crf_showtitle="checked";

		if($this->options['custom_citystatecountry'])
	        $crf_customcitystatecountry="checked";
		if($this->options['custom_anonymous'])
	        $crf_customanonymous="checked";
            ...
            <div id="wpcr_ad">
                <form method="post" action="">
                    ...
                    <div style="padding:10px;padding-bottom:0px;">
						<span style="color:#BE5409;">You can now use this plugin on multiple pages. You will find a "Flowline Customer Reviews" settings box when editing any page.</span>
                        <br /><br />
						...
						<small>If using the "Product" type, you can enter the product name in the "Flowline Customer Reviews" box when editing your pages. If this is set to "Business", the plugin will present all reviews as if they are reviews of your business as listed above.</small>
						<br /><br />
                        ...
                        <br /><br />
                        <label for="show_date">Show Date: </label><input '.$crf_showdate.' class="show_date" id="show_date" name="show_date" type="checkbox" value="1" />
                        <br /><br />
                        <label for="show_date">Show Title: </label><input '.$crf_showtitle.' class="show_title" id="show_title" name="show_title" type="checkbox" value="1" />
                        <br /><br />
                        <div class="submit" style="padding:10px 0px 0px 0px;"><input type="submit" class="button-primary" value="Save Changes" name="Submit"></div>
                    </div>
                    <div style="background:#eaf2fa;padding:6px;border-top:1px solid #ccc;border-bottom:1px solid #ccc;">
                            <legend>Reviews Settings</legend>
                    </div>
                    <div style="padding:10px;padding-bottom:0px;">
						<span style="color:#BE5409;">Options relating to the Reviews.</span>
                        <br /><br />
                        <label for="show_date">Use Anonymous on Name if Empty: </label><input '.$crf_customanonymous.' class="custom_anonymous" id="custom_anonymous" name="custom_anonymous" type="checkbox" value="1" />
                        <br /><br />
                        <label for="show_date">Use Custom City State Country: </label><input '.$crf_customcitystatecountry.' class="custom_citystatecountry" id="custom_citystatecountry" name="custom_citystatecountry" type="checkbox" value="1" />
                        <br /><br />
						<label for="custom_city">Use Custom City Field:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="custom_city" name="custom_city" value="'.$this->options['custom_city'].'" />
						<br /><br />
						<label for="custom_state">Use Custom State Field:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="custom_state" name="custom_state" value="'.$this->options['custom_state'].'" />
						<br /><br />
						<label for="custom_country">Use Custom Country Field:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="custom_country" name="custom_country" value="'.$this->options['custom_country'].'" />
                        <br />
                        <div class="submit" style="padding:10px 0px 0px 0px;"><input type="submit" class="button-primary" value="Save Changes" name="Submit"></div>
						<br /><br />
                    </div>
                    ...
                    <div style="padding:10px;padding-bottom:0px;">
						<span style="color:#BE5409;">Options relating to the Facebook Connect.</span>
						<br /><br />
						<label for="facebook_apisecret">Facebook API Secret:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="facebook_apisecret" name="facebook_apisecret" value="'.$this->options['facebook_apisecret'].'" />
						<br /><br />
						<label for="facebook_apiappid">Facebook API App ID:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="facebook_apiappid" name="facebook_apiappid" value="'.$this->options['facebook_apiappid'].'" />
						<br />
						<br />
						<label for="facebook_apisessionkey">Facebook API Session Key:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="facebook_apisessionkey" name="facebook_apisessionkey" value="'.$this->options['facebook_apisessionkey'].'" />
						<br /><br />
						<label for="facebook_apiuserid">Facebook API User ID:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="facebook_apiuserid" name="facebook_apiuserid" value="'.$this->options['facebook_apiuserid'].'" />
						<br /><br />
						<label for="facebook_apipageid">Facebook Fan Page ID:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="facebook_apipageid" name="facebook_apipageid" value="'.$this->options['facebook_apipageid'].'" />
						<br /><br />
						<label for="facebook_publishtoprofile">Automatic Publishing:</label>&nbsp;&nbsp;
						<input id="facebook_publishtoprofile" name="facebook_publishtoprofile" type="checkbox" '.$facebook_publishtoprofile.' value="1" />&nbsp;Profile Page&nbsp;&nbsp;
						<input id="facebook_publishtopage" name="facebook_publishtopage" type="checkbox" '.$facebook_publishtopage.' value="1" />&nbsp;Fan Page
						<br /><br />
						'.$wpcr_facebookconnect.'
						<br /><br />
                        <div class="submit" style="padding:10px 0px 0px 0px;"><input type="submit" class="button-primary" value="Save Changes" name="Submit"></div>
					</div>
                    <div style="background:#eaf2fa;padding:6px;border-top:1px solid #ccc;border-bottom:1px solid #ccc;">
                    ...
                    <div style="padding:10px;padding-bottom:0px;">
						<span style="color:#BE5409;">Options relating to the Twitter Connect.</span>
                        <br /><br />
						...
						<label for="twitter_ckey">Twitter Consumer Key:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="twitter_ckey" name="twitter_ckey" value="'.$this->options['twitter_ckey'].'" />
						<br /><br />
						<label for="twitter_csecret">Twitter Consumer Secret:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="twitter_csecret" name="twitter_csecret" value="'.$this->options['twitter_csecret'].'" />
						<br /><br />
						<!--
						<label for="autotweet_name">Twitter Tweet Name:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="autotweet_name" name="autotweet_name" value="'.$this->options['autotweet_name'].'" />
						<br /><br />
						<label for="autotweet_token">Twitter Tweet Token:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="autotweet_token" name="autotweet_token" value="'.$this->options['autotweet_token'].'" />
						<br /><br />
						<label for="autotweet_secret">Twitter Tweet Secret:</label>&nbsp;&nbsp;<input style="width:200px;" type="text" id="autotweet_secret" name="autotweet_secret" value="'.$this->options['autotweet_secret'].'" />//-->
						'.$wpcr_twitterconnect.'
						<br /><br />
                        <div class="submit" style="padding:10px 0px 0px 0px;"><input type="submit" class="button-primary" value="Save Changes" name="Submit"></div>
					</div>
                    <div style="background:#eaf2fa;padding:6px;border-top:1px solid #ccc;border-bottom:1px solid #ccc;">
                            <legend>Linkedin Settings</legend>
                    </div>
                </form>
                <br />
            </div>
        ...
        if (isset($this->p->action)) {
            ...
            if (isset($this->p->r)) {
                ...
                switch ($this->p->action) {
					case 'publishfb':
						wpcr_publishfeed($this->p->r);
						break;
					case 'publishtwitter':
						wpcr_stc_publish_automatic($this->p->r);
						break;
                    case 'deletereview':
                      ...
                      <tr class="approved" id="review-<?php echo $rid;?>">
                        ...
                        <td class="comment column-comment">
                          ...
                          <div class="row-actions">
                            <span class="edit"><a title="Edit"
                            href="admin-ajax.php?page=wpcr_view_reviews&amp;action=custreview&amp;task=edit&amp;r=<?php echo $rid;?>&amp;TB_iframe=true" class="thickbox">
                            Edit</a>&nbsp;|&nbsp;<span class="approve <?php if ($review->status == 0 || $review->status == 2) { echo 'wpcr_show'; } else { echo 'wpcr_hide'; }?>"><a title="Mark as Approved"
							...
							<span class="publishfb"><a title="Edit"
                            href="?page=wpcr_view_reviews&amp;action=publishfb&amp;r=<?php echo $rid;?>">
                            Publish to FB</a>&nbsp;|&nbsp;
							<span class="publishfb"><a title="Edit"
                            href="?page=wpcr_view_reviews&amp;action=publishtwitter&amp;r=<?php echo $rid;?>">
                            Publish to Twitter</a>
                          </div>
                        </td>
                      </tr>
                  <?php
