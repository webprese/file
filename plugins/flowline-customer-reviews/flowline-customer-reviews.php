<?php
/* 
 * Plugin Name: Flowline Customer Reviews
 * Plugin URI: http://flowlinedesigns.com/
 * Description: Flowline Customer Reviews allows your customers and visitors to leave reviews or testimonials of your services. Reviews are Microformat enabled (hReview).
 * Version: 2.7.0
 * Author: Flowline, LLC
 * Author URI: http://flowlinedesigns.com/
 *
 * 
 */
require_once("flowline-customer-reviews-additional.php");
global $wpcr_reviewratingtext;
$wpcr_reviewratingtext=Array("Poor","Fair","Good","Very Good","Superb");
...
class WPCustomerReviews
{
	...
	/* keep out of admin file */
	function addmenu() {		
        add_options_page('Flowline Customer Reviews', '<img src="'.$this->getpluginurl().'star.png" />&nbsp;Flowline Customer Reviews', 'manage_options', 'wpcr_options', array(&$this, 'admin_options'));
        add_menu_page('Flowline Customer Reviews', 'Flowline Customer Reviews', 'edit_others_posts', 'wpcr_view_reviews', array(&$this, 'admin_view_reviews'), $this->getpluginurl().'star.png', 50); /* 50 should be underneath comments */
    ...
    function get_options() { 
        ...
        $default_options = array(
			...
			'facebook_apisecret'  => '',
			'facebook_apiappid'  => '',
			'facebook_apisessionkey'  => '',
			'facebook_apiuserid'  => '',
			'facebook_apipageid'  => '',
			'facebook_publishtoprofile'  => '0',
			'facebook_publishtopage'  => '0',
			'twitter_ckey'  => '',
			'twitter_csecret'  => '',
			'autotweet_name'  => '',
			'autotweet_token'  => '',
	...
	function enqueue_scripts() {
		...
		if ($is_active_page && is_singular()) /* make sure its a single page/post and not a list of posts */
		{
            ...
            wp_register_script('flowline-customer-reviews',$this->getpluginurl().'flowline-customer-reviews.js',array(),$this->plugin_version);
            wp_enqueue_script('flowline-customer-reviews');
	...
	function enqueue_styles() {
		...
		wp_register_style('flowline-customer-reviews',$this->getpluginurl().'flowline-customer-reviews.css',array(),$this->plugin_version);        
    ...
    function do_the_content($original_content) {
		...
		{   
            ...
            foreach ($reviews as $review)
            {
                ...
                if ($review->reviewer_name == '' && $this->options["custom_anonymous"]) { $review->reviewer_name = 'Anonymous'; }
				...
				foreach ($this->options['field_custom'] as $i => $val) {
					...
					if ($show == 1 && $custom_fields_unserialized[$val] != '') {
						...
						$custom_i = "custom_$i";
						$custom_shown .= $custom_fields_unserialized[$val].' | ';
					}
				}
				...
				$name_block = ''.
					...
					'</div>';
				*/
				$name_block='<div class="wpcr_fl wpcr_rname">';
				if($this->options['show_date']){
					if($review->date_time!="0000-00-00 00:00:00")
						$name_block .= '<abbr title="'.$this->iso8601(strtotime($review->date_time)).'" class="dtreviewed">'.date("M d, Y",strtotime($review->date_time)).'</abbr>&nbsp;';
				}
				if($this->options['show_title'] && $this->options["custom_anonymous"]){
					$name_block .= '<span class="'.$hide_name.'">by</span>&nbsp;<span class="reviewer vcard" id="hreview-wpcr-reviewer-'.$review->id.'">'.'<span class="fn '.$hide_name.'">'.$review->reviewer_name.'</span>'.'</span>';
				}
				$name_block.=$custom_shown.'</div>';
			
				if ($this->options['hreview_type'] == 'product')
				{
					$reviews_content .= '
						<div class="hreview" id="hreview-'.$review->id.'">
							<'.$title_tag.' class="summary '.$hidesummary.'">'.stripslashes($review->review_title).'</'.$title_tag.'>
				...
				else if ($this->options['hreview_type'] == 'business')
				{
                    ...
                    <div id="hreview-'.$review->id.'">
                        ...
                        '.$name_block.'
                        <div class="wpcr_clear wpcr_spacing1"></div>
						<div class="hreviewdata">
							<div class="hreview">
							   <span class="item">
								  <span class="fn">'.$this->options['business_name'].'</span>
							   </span>
							   Reviewed by <span class="reviewer">'.$review->reviewer_name.'</span> on
							   <span class="dtreviewed">
								  '.date("M d, Y",strtotime($review->date_time)).'<span class="value-title" title="'.date("Y-d-m",strtotime($review->date_time)).'"></span>
							   </span>.
							   <span class="summary">'.stripslashes($review->review_title).'</span>
							   <span class="description">'.stripslashes($review->review_text).'</span>
							   Rating: 
							   <span class="rating">'.$review->review_rating.'</span>
							</div>
						</div>
                        <blockquote class="description"><p>'.stripslashes($review->review_text).'</p></blockquote>
        ...
        $the_content .= '<div class="wpcr_clear wpcr_power">Powered by <strong><a href="http://flowlinedesigns.com/" target="_blank">Flowline, LLC</a></strong></div>';
        $the_content .= '</div>';
        ...
        $out3 = '
                                    ...
                                    <tr>
                                        <td colspan="2" id="wpcr_check_confirm">
                                            ...
                                            <div class="wpcr_fl"><input type="checkbox" name="'.$rand_prefixes[7].'-fconfirm2" id="fconfirm2" value="1" /></div><div class="wpcr_fl" style="margin:-2px 0px 0px 5px"><label for="fconfirm2">Check this box to show you are a person,  not an automated service.</label></div>
                                            <div class="wpcr_clear"></div>
    ...
    function add_review($pageID) {
        ...
        @wp_mail( get_bloginfo('admin_email'), get_bloginfo('name')." Reviews: New Review Posted on ".date('m/d/Y h:i'), "A new review has been posted for ".$this->options['business_name']." via Flowline Customer Reviews. \n\nYou will need to login to the admin area and approve this review before it will appear on your site.\n\n{$admin_link}");
	...
	function include_admin() {
		global $WPCustomerReviewsAdmin;
		require_once(@$this->getplugindir.'flowline-customer-reviews-admin.php'); /* include admin functions */
	}
