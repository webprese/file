...
class Flowline_Woo_Widget_Subscribe extends WP_Widget {
	...
	function Flowline_Woo_Widget_Subscribe () {
		
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_woo_subscribe', 'description' => __( 'Add a subscribe/connect widget.', 'woothemes' ) );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'flowline_woo_subscribe' );

		/* Create the widget. */
		$this->WP_Widget( 'flowline_woo_subscribe', __('Flowline Woo - Subscribe / Connect', 'woothemes' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {  
		...
		if ( ! is_singular() || ( $single == 'on' && is_single() ) || ( $page == 'on' && is_page() ) ) {
			...
			flowline_woo_subscribe_connect( 'true', $title, $form, $social );
...
add_action( 'widgets_init', create_function( '', 'return register_widget("Flowline_Woo_Widget_Subscribe");' ), 1 ); 
?>